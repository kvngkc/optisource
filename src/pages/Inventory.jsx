...
export default function Inventory() {
  ...
  async function fetchRecentLogs() {
    const { data } = await supabase.from('transactions').select('*, products(name), locations(name, code)').eq('company_id', profile.company_id).eq('type', 'INVENTORY_ADD').order('created_at', { ascending: false }).limit(8)
    setRecentLogs(data || [])
  }
  ...
  async function handleSubmit(e) {
    ...
    if (existing) {
      ...
      const { error } = await supabase.from('stock').insert({ company_id: profile.company_id, product_id: form.product_id, location_id: form.location_id, ...specs, qty })
      stockError = error
    }
    if (stockError) { flash('error', stockError.message); setLoading(false); return }
    await supabase.from('transactions').insert({ company_id: profile.company_id, type: 'INVENTORY_ADD', product_id: form.product_id, location_id: form.location_id, ...specs, qty, created_by: profile.id })
    await supabase.from('audit_log').insert({ company_id: profile.company_id, user_id: profile.id, status: 'SUCCESS', action: 'INVENTORY_ADD', details: { product: selectedProduct?.name, location: locations.find(l => l.id === form.location_id)?.code, ...specs, qty } })
    ...
    <Layout>
      <div className="max-w-2xl mx-auto px-4 py-6 lg:px-6 lg:py-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="number" min="1" required value={form.qty}
                onChange={e => update('qty', e.target.value)} placeholder="e.g. 50"
        ...
        <div className="space-y-2">
          ...
          {recentLogs.map(log => (
            <div key={log.id} className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center justify-between">
              <div>
                <p className="font-medium text-slate-800 text-sm">{log.products?.name}</p>
                <p className="text-xs text-slate-400 mt-0.5">
                  {log.locations?.code} · {log.sph || log.name_key || '—'}{log.addition ? ' / ' + log.addition : ''}
                </p>
              </div>
